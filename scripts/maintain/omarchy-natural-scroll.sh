...
PROBE="$(cd "$(dirname "$0")" && pwd)/hypr-option.lua"
...
# hyprctl talks to the instance in HYPRLAND_INSTANCE_SIGNATURE, which a shell
# (or tmux server) that outlived a re-login still points at the dead one, and
# it exits 0 even when it can't connect. So pick an instance that answers:
# the environment's, else the newest. Prints nothing when none does.
live_instance() {
